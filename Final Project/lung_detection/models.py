...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        ce_loss = F.binary_cross_entropy_with_logits(inputs, targets, weight=class_weights, reduction='none')
...
class ClassifierHead(nn.Module):
    ...
    def __init__(self, head_n_layer=3, head_n_head=8, head_feature_map_dim=31,
                 ...
                 head_dropout=0.01, head_classifier_input_features = 2048, head_hidden_size=1024, 
                 use_vit=False, num_classes=14, **kwargs):
        super().__init__()
        ...
        self.classifier_fc = nn.Sequential(
            ...
            nn.Linear(head_hidden_size//64,14),
        ...
        if use_vit:
            self.vit = ViT(
                            image_size = head_feature_map_dim,
                            patch_size = 5,
...
class XRayPredictor(nn.Module):
    ...
    def __init__(self, fine_tune = False, **kwargs):
        super().__init__()
        self.trunk = ClassifierTrunk(**kwargs)
        self.head = ClassifierHead(**kwargs)

        if fine_tune:
            checkpoint = torch.load('../data_processing/model.pth.tar', map_location=torch.device('cuda:0'))
            #loading the dictionary of the checkpoint and loading the densent model
            dense_model = models.densenet121(pretrained=True).cuda()
    ...
    def forward(self, input_embeds):
        if hasattr(self, 'dense_model'):
            input_embeds = self.dense_model.features(input_embeds)
...
class XRayLightning(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_accuracy', train_accuracy, batch_size=batch_size, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_accuracy', val_accuracy, batch_size=batch_size, on_step=False, on_epoch=True, prog_bar=True)
        self.log('val_f1_score', f1_score_avg, batch_size=batch_size, on_step=False, on_epoch=True, prog_bar=True)
        self.log('val_weighted_f1', weighted_f1, batch_size=batch_size, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def configure_optimizers(self):
        ...
        scheduler = CosineAnnealingLR(optimizer, T_max=4, eta_min=0.001)
    ...
    def _process_batch(self, batch, compute_accuracy=False):
        ...
        if compute_accuracy:
            ...
            weighted_f1_score_metric = torchmetrics.F1Score(task="multilabel",
                                                            num_labels=num_labels, average='weighted').cuda()
            weighted_f1_score = weighted_f1_score_metric(logits, labels)

            # pytorch auroc scores per class
            auroc_metric = torchmetrics.AUROC(task="multilabel", num_labels=num_labels,
                                       average="none").cuda()
